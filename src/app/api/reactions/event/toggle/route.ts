...
export async function POST(request: NextRequest) {
  try {
    ...
    const input = parseEventLikeToggleInput(body);
    const result = await toggleEventLike(input, actor.actorId);
    ...
    if (result.status === "event_not_found") {
      return NextResponse.json({ error: "Event not found in meet." }, { status: 404 });
    }
